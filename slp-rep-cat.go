...
// Authors: Dhairyya Singh, Anna Schapiro

// slp-rep-cat runs a hippocampal-cortical model on the satellite category learning task (see Schapiro et al. (2017))
package main
...
type Sim struct {
	...
	SlpTestWrtOut bool              `desc:"Write out Test Stats Pre-Post Sleep? Set to false to reduce disk space consumption"`
	LrnActsWrtOut bool              `desc:"Write out Learn Acts? Set to false to reduce disk space consumption"`
	...
	// internal state - view:"-"
	// Need separate Shared and Unique feature sums for tracking within epcs
...
func (ss *Sim) New() {
	...
	ss.LayStatNms = []string{"F1", "F2", "F3", "F4", "F5", "ClassName", "CodeName", "pCA1", "dCA1", "CTX", "DG"}
	...
	ss.SlpTrlOcc = false
	ss.SlpWrtOut = false
	ss.SlpTestWrtOut = false
	ss.LrnActsWrtOut = false
}
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	// Magic CTX
	ctx := net.AddLayer2D("CTX", 15, 15, emer.Hidden)
	...
	for _, lyc := range PerLays {
		...
		pj = net.ConnectLayersPrjn(ly, dca1, conn, emer.Forward, &hip.CHLPrjn{}) 
		pj.SetClass("PerCA1Prjn")
...
func (ss *Sim) SleepCycInit() {
	...
	// Initial injection of random activity
	for _, ly := range ss.Net.Layers {
	...
	// Synaptic depression parameters defined here
	if ss.SynDep {
		for _, ly := range
			ss.Net.Layers {
			inc := 0.00035
...
func (ss *Sim) AlphaCyc(train bool) {
	...
	if ss.LrnActsWrtOut{ // If true, write out of all train trial acts - output file sizes are big!
		if !train {
			dirpathacts := "output/" + "lrnacts" + "/" + "tstacts" + fmt.Sprint(ss.DirSeed) + "_truns_" + fmt.Sprint(ss.MaxRuns) + "_run_" + fmt.Sprint(ss.TrainEnv.Run.Cur)
	
			if _, err := os.Stat(filepath.FromSlash(dirpathacts)); os.IsNotExist(err) {
				os.MkdirAll(filepath.FromSlash(dirpathacts), os.ModePerm)
			}
	
			filelrnacts, _ := os.OpenFile(filepath.FromSlash(dirpathacts+"/"+"lrnacts"+fmt.Sprint(ss.RndSeed)+"_"+"run"+fmt.Sprint(ss.TrainEnv.Run.Cur)+"epoch"+fmt.Sprint(ss.TrainEnv.Epoch.Cur)+".csv"), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
			...
			if ss.TrainEnv.Epoch.Cur == 1 {
				...
				mainfile, err := ioutil.ReadFile("slp-rep-cat.go")
				if err != nil {
					fmt.Println(err)
					return
				}
	
				err = ioutil.WriteFile(dirpathacts+"/"+fmt.Sprint(ss.DirSeed)+"slp-rep-cat.go", mainfile, 0644)
				if err != nil {
					fmt.Println("Error creating", dirpathacts+"/"+fmt.Sprint(ss.DirSeed)+"_"+"params.go")
					fmt.Println(err)
					return
...
func (ss *Sim) TrainTrial() {
	...
	if chg {
		...
		if ss.TestInterval > 0 && epc%ss.TestInterval == 0 { // note: epc is *next* so won't trigger first time
			...
			// Training criterion defined here
			if ss.EpcShPctCor >= 0.66 && ss.EpcUnPctCor >= 0.66 {
				ss.TestAll(true) // Extra test right before sleep - results written to slplearning dir
								 // Can use this to do lesion testing in order to not slow down the entire sim

				if ss.ExecSleep {
					dirpathslp := "output/" + "slpacts/" + fmt.Sprint(ss.DirSeed) + "/"
					if _, err := os.Stat(filepath.FromSlash(dirpathslp)); os.IsNotExist(err) {
						os.MkdirAll(filepath.FromSlash(dirpathslp), os.ModePerm)
					}
					fileslpres, _ := os.OpenFile(filepath.FromSlash(dirpathslp+"slpres.csv"), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
					...
					results := []string{strconv.FormatFloat(ss.EpcShPctCor, 'f', 6, 64), strconv.FormatFloat(ss.EpcUnPctCor, 'f', 6, 64), strconv.FormatFloat(ss.EpcShSSE, 'f', 6, 64), strconv.FormatFloat(ss.EpcUnSSE, 'f', 6, 64)}
					...
					ss.TestAll(true)
					results = []string{strconv.FormatFloat(ss.EpcShPctCor, 'f', 6, 64), strconv.FormatFloat(ss.EpcUnPctCor, 'f', 6, 64), strconv.FormatFloat(ss.EpcShSSE, 'f', 6, 64), strconv.FormatFloat(ss.EpcUnSSE, 'f', 6, 64), strconv.Itoa(ss.SlpTrls / 10)}
					writerslpres.Write(results)
					writerslpres.Flush()
					fileslpres.Close()
					ss.FinalTest = false
...
// Runs one 30k cycle sleep epoch
func (ss *Sim) SleepCyc(c [][]float64) {
	...
	filetrnacts, _ := os.OpenFile(".csv", os.O_APPEND, 0644)
	writertrnacts := csv.NewWriter(filetrnacts)

	if ss.SlpWrtOut{ // If true, write out of all sleep epoch acts - output file sizes are big!
		filetrnacts, _ = os.OpenFile("output/"+"slpacts/"+fmt.Sprint(ss.DirSeed)+"/"+"trnacts"+fmt.Sprint(ss.RndSeed)+"_"+"run"+fmt.Sprint(ss.TrainEnv.Run.Cur)+"epoch"+fmt.Sprint(ss.TrainEnv.Epoch.Cur)+".csv", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	...
	for cyc := 0; cyc < 30000; cyc++ {
		...
		// Taking the prepared slice of oscil inhib values and producing the oscils in all perlys := []string{"F1", "F2", "F3", "F4", "F5", "CodeName", "ClassName"}
		if ss.InhibOscil {
			inhibs := c
			if inhibs[0][cyc] > 1.05 {
				ss.InhibFactor = 1.05 // For sleep GUI counter and sleepcyclog
			} else {
			...
			for _, layer := range lowlayers {
				if inhibs[0][cyc] > 1.05 {
					...
					ly.Inhib.Layer.Gi = ly.Inhib.Layer.Gi * float32(1.05)
				} else {
					ly := ss.Net.LayerByName(layer).(*leabra.Layer)
		...
		ss.AvgLaySim = avesim / 12

		//If AvgLaySim falls below 0.8 - most likely because a layer has lost all act, random noise will be injected
		...
		if ss.SlpLearn {
			...
			// These thresholds define how stable the network has to be in plus and minus phases
			plusthresh := 0.999965             
			...
			if stablecount == 5 && ss.AvgLaySim >= plusthresh && ss.PlusPhase == false && ss.MinusPhase == false { //&& ss.Time.Quarter == 0 {
				...
				stablecount = 0
				minuscount = 0
				ss.PlusPhase = true // plusphase begins
				...
				ss.PlusPhase = false // plusphase ends as network has fallen below plusthresh
				ss.MinusPhase = true
				minuscount++

				// calling function here which will calculate final plusphase act avg for all synapses and store in syn var
				for _, ly := range ss.Net.Layers {
				...
				ss.MinusPhase = false // minusphase ends as network has fallen below minusthresh

				// calling function here which will calculate final minusphase act avg for all synapses and store in syn var
				for _, ly := range ss.Net.Layers {
...
func (ss *Sim) RunEnd() {
	//ss.LogRun(ss.RunLog) //commenting out to first only figure out run epc log output
	if ss.SaveWts {
...
func (ss *Sim) TrialStats(accum bool, outlaynm string) (sse, avgsse, cosdiff float64) {
	...
	if accum {
		if ss.HiddenType == "shared" && ss.TestEnv.Trial.Cur >= 0 && ss.TestEnv.Trial.Cur < 105 { // note the trial accumulation numbers here are for only first 105 trials to not get the TSP and CTX separate trials in
...
func (ss *Sim) TestTrial(returnOnChg bool, slptest bool) {
	...
	if ss.SlpTestWrtOut{ // If true, write out pre and post sleep test epochs - useful for stats
		if slptest{
			dirtrlstats := "output/" + "slplearning/" + fmt.Sprint(ss.DirSeed) + "/"
			if _, err := os.Stat(filepath.FromSlash(dirtrlstats)); os.IsNotExist(err) {
				os.MkdirAll(filepath.FromSlash(dirtrlstats), os.ModePerm)
			}
	
			filetrlstats, _ := os.OpenFile(filepath.FromSlash(dirtrlstats+"/"+"trlststats"+fmt.Sprint(ss.RndSeed)+".csv"), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
			...
			valueStr := []string{fmt.Sprint(ss.RndSeed), fmt.Sprint(ss.TestEnv.TrialName.Cur), fmt.Sprint(ss.TrlSSE), fmt.Sprint(ss.TrlAvgSSE), fmt.Sprint(ss.TrlSSE == 0), fmt.Sprint(ss.HiddenType), fmt.Sprint(ss.HiddenFeature)}
...
func (ss *Sim) TestItem(idx int) {
	...
	//ss.TrialStats(false, outlay) // !accumulate
...
func (ss *Sim) TestAll(slptest bool) {
	...
	lesion := 1
	if slptest{
		lesion = 5 // skipping last 2 now
	} else {
		lesion = 1
	}

	for k := 0; k < lesion; k++ { // k < 3 for full lesion testing
		if k == 1 {
			ctx.SetOff(true)
			ss.Net.GScaleFmAvgAct() 
			ss.Net.InitGInc()       
		}
		...
		for i := 0; i < 7; i++ { // i < 7
			for j := 0; j < 15; j++ {
				...
				ss.LogTstTrl(ss.TstTrlLog) // Note: this is added here because of the way in which outlay for specific trials get passed around and identified. Ideally this
				// This needs to be put back at the end of testtrial() but that will require fixing of ss.HiddenFeature and less passing of args between funcs.
				// This works for now, but fix later.
...
func (ss *Sim) LogTrnEpc(dt *etable.Table) {
	...
	//fmt.Println(shnt, unnt)
	//fmt.Println(ss.ShCntErr, ss.UnCntErr)
...
func (ss *Sim) LogTstEpc(dt *etable.Table) {
	...
	shnt := float64(78)                 //shnt := float64(ss.ShTrlNum) changed
	unnt := float64(27)                 // float64(ss.UnTrlNum) changed
